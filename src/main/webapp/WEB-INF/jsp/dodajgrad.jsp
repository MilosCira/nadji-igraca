                ...
                <div class="row">
                    ...
                    <div class="container">
                        <div class="panel-body">
                            <c:choose>
                                <c:when test="${izmena}">
                                    <form id="register-form" action="http://localhost:8080/NadjiIgraca/admin/updategrad?id=${grad.id}" method="post" role="form">
                                    </c:when>
                                    <c:otherwise>    
                                        <form id="register-form" action="http://localhost:8080/NadjiIgraca/admin/dodajgrad" method="post" role="form">
                                        </c:otherwise>
                                    </c:choose> 
                                    <div class="form-group">
                                        <input type="text" name="ime" id="ime" tabindex="1"  class="form-control" placeholder="Ime grada" value="${grad.ime}" required>
                                    </div>
                                        <c:if test="${izmena==null}">
                                    <select name="drzava">
                                        <c:forEach items="${drzave}" var="drzava">
                                            <option value=<c:out value="${drzava.id}"/> > <c:out value="${drzava.ime}"/></option>
                                        </c:forEach>
                                    </select>
                                        </c:if>
                                    </div> 
                                        ...
                                        <div class="row">
                                            <c:if test="${izmena==null}">
                                                <div class="col-sm-6 col-sm-offset-3">
                                                    <input type="submit" name="register-submit" id="register-submit" tabindex="4" class="form-control btn btn-register" value="Dodaj Grad">
                                                </div>
                                            </c:if>
                                            <c:if test="${izmena}">
                                                <div class="col-sm-6 col-sm-offset-3">
                                                    <input type="submit" name="register-submit" id="register-submit" tabindex="4" class="form-control btn btn-register" value="Izmeni Grad">
                                                </div>
                                            </c:if>
                                        </div>
                                    </div>
